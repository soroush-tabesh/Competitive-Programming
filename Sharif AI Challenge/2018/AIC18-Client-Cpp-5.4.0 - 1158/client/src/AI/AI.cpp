...
void simple_attack(int gold) {
    ...
    if(wold->get_current_turn()<700 || wold->get_current_turn()%5==0 || burst) {
        vector<int> paths;
        int po = 0;
        ...
        if (wold->get_current_turn() % 210 == 0 || burst || (wold->get_current_turn()>700 && wold->get_current_turn()%20==0)) {
...
void AI::simple_turn(World* world) {
    ...
    if (!burst) {
        int money1 = residue+(money-residue-residue2) * (5 - (world->get_current_turn() +(world->get_current_turn()>burst_num)*(100))/ 400) / 10;
